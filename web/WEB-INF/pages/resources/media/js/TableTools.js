...
TableTools.prototype = {
	...
	{
              console.log("calling tabletools :: report name = " + oConfig.sTitle);
		var
			aoCols = this.s.dt.aoColumns,
			aColumnsInc = this._fnColumnTargets( oConfig.mColumns ),
			aColWidths = [],
                        //temp = [0.05,0.05,0.03,0.045,0.08,0.07,0.125,0.05,0.05,0.05,0.05,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.05,0.05,0.05],
			domBackBon = [0.05,0.05,0.04,0.045,0.07,0.065,0.14,0.055,0.055,0.05,0.05,0.035,0.035,0.035,0.035,0.035,0.045,0.045,0.04,0.05,0.05],
                        intrBackBon = [0.05,0.05,0.04,0.045,0.07,0.065,0.14,0.055,0.055,0.05,0.05,0.035,0.035,0.035,0.035,0.035,0.045,0.045,0.04,0.05,0.05],        
                        peerLinkUtil=[0.05,0.05,0.055,0.04,0.04,0.07,0.065,0.135,0.04,0.055,0.055,0.055,0.055,0.055,0.055,0.05,0.05,0.04,0.04],
                        unusedPolicy=[0.05,0.03,0.14,0.06,0.06,0.06,0.06,0.55],
                        unusedVrf=[0.05,0.03,0.14,0.06,0.06,0.06,0.06,0.55],
                        vrfUtil=[0.05,0.05,0.05,0.05,0.15,0.05,0.2,0.08,0.07,0.06,0.06,0.05,0.05],
                        routerIfUtil=[0.05,0.05,0.035,0.04,0.09,0.08,0.15,0.055,0.055,0.055,0.055,0.045,0.045,0.045,0.045,0.045,0.05,0.05],
                        classOfService=[0.06,0.06,0.06,0.04,0.035,0.15,0.12,0.08,0.14,0.04,0.06,0.06,0.06,0.06],
                        lspPath = [0.06,0.06,0.2,0.08,0.13,0.13,0.08,0.05,0.08,0.08,0.07],
		...
		for ( i=0, iLen=aColWidths.length ; i<iLen ; i++ )
		{
			aColWidths[i] = aColWidths[i] / iTotal;
                ...
                if(oConfig.sTitle=="Domestic Backbone Service wise Link Utilization Report")
                {
                    console.log("inside dom backbone if condition");
                   return domBackBon.join('\t'); 
                }
                else if(oConfig.sTitle=="International Backbone Service wise Link Utilization Report")
                {
                    return intrBackBon.join('\t');
                }
                else if(oConfig.sTitle=="Domestic/International Peering Link Utilization Report")
                {
                    return peerLinkUtil.join('\t');
                }
                else if(oConfig.sTitle == "Unused Service Policy Report")
                {
                    return unusedPolicy.join('\t');
                }
                else if(oConfig.sTitle == "Unused VRF Report")
                {
                    return unusedPolicy.join('\t');
                }
                else if(oConfig.sTitle == "VRF Utilization Report")
                {
                    return vrfUtil.join('\t');
                }
                else if(oConfig.sTitle=="Router wise Interface Utilization Report")
                {
                    return routerIfUtil.join('\t');
                }
                else if(oConfig.sTitle == "IP/MPLS Core Network Class of Service Traffic")
                {
                    return classOfService.join('\t');
                }
                else if(oConfig.sTitle == "LSP Path Report")
                {
                    return lspPath.join('\t');
                }
		else
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
					...
					sLoopData = this._fnHtmlDecode( sLoopData );
                                         if(oConfig.sAction=='flash_pdf')
                                        {
                                            sLoopData=sLoopData.replace(/\s+/g,' ');
                                        } 
		...
		for ( j=0, jLen=dt.aiDisplay.length ; j<jLen ; j++ )
		{
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
					...
					sLoopData = this._fnHtmlDecode( sLoopData );
					//trim to 100characters
                                      /* 
                                        if(oConfig.sAction=='flash_pdf')
                                        {
                                            
                                            sLoopData=(sLoopData.length >100)?( sLoopData.substring(0,100)+"..."):sLoopData;
 ...
 */
TableTools.DEFAULTS = {
	"sSwfPath":         "media/swf/eightpixel.swf",
	"sRowSelect":       "none",
